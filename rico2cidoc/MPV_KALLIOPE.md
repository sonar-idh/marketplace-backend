...
- End Date (as the Creation event's time-span)
...
| `rico:endDate` (Time-Span) | `?letterURI rico:endDate ?date` | `crm:E52_Time_Span` | `crm:P4_has_time-span` (on the Creation Event) | Time-span of the *creation* event, derived from the record's end date. URI: `kpe:{letterId}#TimeSpan` |
| `rico:endDate` (Time Primitive) | same pattern | `crm:E61_Time_Primitive` | `crm:P170i_time_is_defined_by` | Value is truncated to year precision (`SUBSTR(?date, 1, 4)`) even though the source `?date` is a full `xs:date`. Note: only `rico:endDate` is read; `rico:beginningDate` is not part of this query, so date ranges collapse to a single year taken from the end date. *Should be discussed.* |

> **Note:** Time-span modeling only covers a record's `rico:endDate`, truncated to year precision; `rico:beginningDate` and full-precision/range time-spans are not yet modeled.
