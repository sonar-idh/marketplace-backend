...
<xsl:stylesheet version="3.0"
  ...
  <xsl:function
    ...
    <xsl:choose>
      ...
      <xsl:when test="$y eq 0 or $m gt 12 or $d gt 31">
        <!-- Moeglicher Edge Case: Was passiert mit Datumsangaben vor dem Jahr Null? Momentan nicht zulaessig. -->
        <xsl:sequence select="()" />
      </xsl:when>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          ...
          <!-- Subcase: Kalendarisch inkorrekter Tag, z.B. 20130231: existiert nicht. Nicht
  ...
  <!-- ==================================================================
       Tripel schreiben.  Zwei getrennte Templates, um (1) Time-Spans an bestehende Sending-Entitäten der
  Briefe anzubinden und (2) Time-Span-, Start-Date- und End-Date-Entitäten anzulegen und zu verknuepfen.
       ================================================================== -->
  ...
  <!-- Time-Span-IRI aus Record-ID und Datumsschluessel erzeugen -->
  <xsl:function
  ...
  <!-- Start-/End-Date-IRI aus Datum und Rollenangabe erzeugen -->
  <xsl:function
    name="f:date-iri" as="xs:string">
    <xsl:param name="d" as="xs:date" />
    <xsl:param name="role" as="xs:string" />
    <xsl:sequence
      select="'sonar:' || $role || '_Date_' || string($d)" />
  </xsl:function>
